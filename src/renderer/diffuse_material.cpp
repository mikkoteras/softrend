...
color4 diffuse_material::shade_flat(const surface_position &point, const scene_render_context &scene) const {
    color3 result(point.shade * (ambient_color_at(point.uv) + diffuse_color_at(point.uv) + emissive_color_at(point.uv)));
    result.clamp();
    return color4(result, get_dissolve());
}

color4 diffuse_material::shade_gouraud(const surface_position &point, const scene_render_context &scene) const {
    color3 result(point.shade * (ambient_color_at(point.uv) + diffuse_color_at(point.uv) + emissive_color_at(point.uv)));
    result.clamp();
    return color4(result, get_dissolve());
}

color4 diffuse_material::shade_phong(const surface_position &point, const scene_render_context &scene) const {
