...
color4 diffuse_material::shade(const surface_position &point, const scene_render_context &scene, bool fast) const {
    ...
    if (fast) {
        color3 result(point.shade * (ambient_multiplier + diffuse_multiplier) + emissive_term);
        result.clamp();
